...
class VandermondeTransform:
    ...
    def make_matrix(self):
        ...
        for row in range(self.modes):
            ...
            #V[:,row,:] = torch.exp(-1j * (row - int(self.modes/2)) * self.positions[:,:])
...
class FNO_dse (nn.Module):
    ...
    configs = {
        ...
        #'width':                8,                     # Number of channels in the convolutional layers
    }
    def __init__(self, configs):
        ...
        #self.conv1 = SpectralConv1d(1, 1, self.modes)
        #self.conv2 = SpectralConv1d(1, 1, self.modes)
        #self.conv0 = SpectralConv1d_dd(self.width, self.width, self.modes)
        ...
        #self.w0 = nn.Conv1d(self.width, 1, 1)
        ...
        #self.fc1 = nn.Linear(1, self.width)
        #self.fc2 = nn.Linear(self.width, 1)
        #self.fc1 = nn.Conv1d(1, self.width, 1)
        #self.fc2 = nn.Conv1d(self.width, 1, 1)
        #self.fc2 = nn.Linear(self.width, self.width)
        #self.fc3 = nn.Linear(self.width, self.width)
        #self.fc4 = nn.Linear(self.width, 1)
        self.fq1 = nn.Linear(1, 8)
        self.fq2 = nn.Linear(8, 1)
    ...
    def forward(self, x):
        ...
        #q = q+x
        #q = F.gelu(q)
        #q = self.conv1(q, transform)
        #q = F.gelu(q)
        #q = self.conv2(q, transform)
        q = q.permute(0,2,1)
