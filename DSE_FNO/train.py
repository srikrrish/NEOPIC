...
configs = {
    ...
    'experiment':           'Landau',               # Burgers, Elasticity, Airfoil, ShearLayer, Humidity
    ...
    'model_path':           '_Models/dt_002_T_10/Landau_fno_dse.pt',         # Path to model file if loading model
...
def train (configs):
    ...
    ### Load Dataset
    try:
        getDataloaders = importlib.import_module(configs['experiment']+'.dataset1d').getDataloaders
    except Exception as error:
        print(error)
    ...
    for epoch in range(configs['epochs']):
        ...
        train_time_epoch = stop_train-start_train
        training_times.append(train_time_epoch)
    ...
    # Save Model
    #kv = torch.arange(17) - int(32/2)
    #h = 4*np.pi / 32
    #mod_shape = torch.squeeze(best_model.conv0.weights).detach().cpu() * (torch.sin(0.25*kv*h)/(0.25*kv*h))**2
    #plt.plot(kv,torch.squeeze(best_model.conv0.weights).cpu().numpy())
    #plt.plot(kv,mod_shape.numpy())
    #plt.savefig('weights_symmetric_sinc_shape.png')
